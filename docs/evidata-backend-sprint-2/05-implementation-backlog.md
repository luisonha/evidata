# 05 — Backlog de implementación backend v1.6.8
...
| P1-SCRIPTS-ALIGNMENT | ✅ COMPLETADO: Alineación de scripts de provisión/seed con el modelo RBAC post-sprint. | **Origen**: auditoría post-cierre RBAC (usuario) sobre `scripts/local/{setup,start,seed,migrate}.sh` y seeders de dominio. **3 problemas confirmados y corregidos**: (1) migración `AddReviewDomainField` duplicaba la creación de la tabla `review_requirements` (ya creada por `AddReviewRequirements`) — habría hecho fallar `dotnet ef database update` en Postgres real; corregida quirúrgicamente para conservar solo el `AddColumn`. (2) `seed.sh` sembraba roles legacy `DPO`/`PrivacyAnalyst` no reconocidos por `TenantOwnerOrComplianceAdminHandler` — el admin de desarrollo no podía gestionar roles ni ReviewRequirements (403); corregido para asignar `TenantOwner`/`ComplianceAdmin` reales vía SQL dinámico, y se limpiaron 10 permisos legacy "Ley 21.719" huérfanos. (3) `GapRuleInitializer` (catálogo de 11 reglas) nunca se invocaba — primer intento vía `HasData()` en migración EF fue revertido por el coordinador (baked un tenant fake en una migración que corre en TODOS los ambientes incl. producción, dejando las reglas huérfanas para tenants reales); solución final: seeding movido exclusivamente a `seed.sh` (solo dev), con nuevo test `GapRuleInitializerTests` (5 tests) verificando integridad del catálogo. **Follow-up**: `smoke-test.sh` referenciaba el roleId legacy de `DPO` eliminado (regresión directa de este PR) — corregido en PR separado inmediato. PR #123, aprobado sin condiciones por Gandalf, merged 2026-07-11. 828 tests totales, 0 regresiones. | ✅ Unit tests (GapRuleInitializerTests); Ninguno |
...
- 📊 **828 unit tests pasando**, 0 regressions (PR #112, #113, #114, #115, #116, #117, #118, #119, #120, #121, #123)
