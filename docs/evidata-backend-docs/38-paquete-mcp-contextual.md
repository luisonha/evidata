# MCP Contextual — Paquete de implementación backend

## 1. Propósito

Este paquete permite implementar **MCP Interactive en core y MCP Batch Jobs como Azure Functions** sin leer todo el set documental. Contiene las reglas mínimas de arquitectura, seguridad, datos, APIs, eventos, infraestructura Azure, pruebas y Definition of Done necesarias para el módulo.
...
El alcance incluido es: **asistente contextual con citas, abstención, HITL, mínimo contexto tenant y guardrails**.
...
`McpInteraction, McpCitation, McpHumanReviewTask, McpRiskAssessment`
